...
Paused output keeps Stalker's actual execution PC separate from the displayed
user-code PC. If Stalker reaches real code in the breakpoint's module, that
address is shown directly. If it first reaches relocated entry code generated by
Frida, the agent maps it back to the pre-attach disassembly snapshot and logs
both `pc=<display address>` and `actual=<stalker address>`.
...
- Add debug recording. A future `record()` flow should capture each pause with
  the displayed PC, actual Stalker PC, instruction, register diff, stack diff,
  and resolved symbols so a debugging session can be exported and reviewed.
- Abstract breakpoint backends. The current implementation uses
  `Interceptor.attach()` as the breakpoint backend. Future backends should
  support `interceptor`, `hardware`, and `software-brk`, letting the debugger
  choose the safest available mechanism for exact instruction breakpoints.
- Enhance `callFunction()`. Planned improvements include explicit return types,
  explicit argument types, automatic JavaScript string conversion through
  `Memory.allocUtf8String()`, and Unity IL2CPP return-value formatting so
  `Il2Cpp.String` results are printed as readable strings.
